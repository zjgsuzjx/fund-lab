...
export function BuyPage({ code, user, onUnauthorized }: Props & { code: string }) {
  ...
  return <main className="market-shell trade-page entry-page"><Heading title="买入基金" back={`fund/${code}`}>仅使用虚拟余额，不发生真实扣款</Heading>
    {loading ? <p role="status">正在加载买入信息…</p> : fund && account && <form onSubmit={submit}>
      ...
      <div className="submit-bar"><button className="primary" disabled={busy || (!uncertain && (!quote || quoting || !valid))}>{busy ? '正在提交…' : uncertain ? '重试并确认原申请结果' : `确认模拟买入 · ${valid ? money(amount) : '—'} 元`}</button><small>使用虚拟余额 · 不发生真实扣款</small></div>
    </form>}
...
function Progress({ order }: { order: Order }) {
  const confirmed = order.status === 'confirmed', cancelled = order.status === 'cancelled'
  return <TradeTimeline cancelled={cancelled} steps={[
    { title: '申请已提交', detail: `${time(order.created_at)} · 虚拟资金已预留`, complete: true },
    { title: confirmed ? '净值已确认' : cancelled ? '停止等待净值' : '等待净值确认', detail: `计价日 ${order.trade_date}${order.confirmed_nav ? ` · 净值 ${Number(order.confirmed_nav).toFixed(4)}` : ''}`, complete: confirmed },
    { title: confirmed ? '份额已到账' : cancelled ? '预留资金已退回' : '等待份额到账', detail: order.completed_at ? time(order.completed_at) : `预计 ${order.confirmation_date} 起确认`, complete: confirmed || cancelled },
  ]} />
}
...
export function TradingOverview({ holdings, user, onUnauthorized }: Props & { holdings: boolean }) {
  ...
  return <main className="market-shell trade-page">{!holdings && <header className="tab-heading"><h1>交易</h1><p className="subtitle">查看申请、确认与资金到账</p></header>}
    ...
    {loading ? <p role="status">正在加载…</p> : error ? <div className="error" role="alert">{error}<button onClick={() => setRetry(v => v + 1)}>重新加载</button></div> : holdings && portfolio ? <Holdings portfolio={portfolio} refresh={retry} onRefresh={() => setRetry(v => v + 1)} /> : orders && <><div className="order-list">{orders.items.length ? orders.items.map(order => <a className="trade-card order-link" key={order.id} href={`#${order.kind === 'sell' ? 'sell-order' : 'order'}/${order.id}`}><div className="order-card-top"><span>{order.kind === 'sell' ? '基金卖出' : '基金买入'}</span><span className={`order-status status-${order.status}`}>{order.kind === 'sell' ? sellStatusName[order.status] : statusName[order.status]}</span></div><h2>{order.fund_name}</h2><Row label={time(order.created_at)}>{order.kind === 'sell' ? `${money(order.shares)} 份` : `${money(order.amount)} 元`} ›</Row></a>) : <section className="market-empty"><h2>暂无相关交易</h2><p>模拟买入后，在这里查看进度与记录。</p><a href="#discover">去发现基金 ›</a></section>}</div><div className="pagination"><button disabled={page <= 1} onClick={() => setPage(v => v - 1)}>上一页</button><span>第 {page} 页 · 共 {orders.total} 笔</span><button disabled={page * 20 >= orders.total} onClick={() => setPage(v => v + 1)}>下一页</button></div></>}
