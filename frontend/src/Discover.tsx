...
export default function Discover({ user, onUnauthorized }: { user: User | null; onUnauthorized: (message: string) => void }) {
  ...
  return <main className="market-shell discover-page">
    <p className="account-tag">模拟账户</p><div className="market-title"><h1>基金练习室</h1><a href={user ? '#account' : '#login'}>{user ? '账户' : '登录'} ›</a></div><p className="subtitle">用虚拟资金，练习每一个投资决定</p>
    <section className="asset-card" aria-label="模拟资产"><span>模拟总资产（元）</span><strong className="asset-total">{account ? formatMoney(account.available_cash) : user ? '—' : '登录后查看'}</strong><div className="asset-columns"><div><span>累计收益</span><strong>{account ? '0.00' : '—'}</strong></div><div><span>可用余额</span><strong>{account ? formatMoney(account.available_cash) : '—'}</strong></div></div>{account && <small>当前尚无交易，资产全部为虚拟现金</small>}{accountError && <p role="alert">{accountError}<button onClick={() => setRefresh(value => value + 1)}>重试</button></p>}</section>
