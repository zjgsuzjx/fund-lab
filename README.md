...
> **当前进度：账户、基金发现、详情与自选已接通。** 支持用户名登录、一次性虚拟本金，以及按 V2 定稿实现的基金搜索 / 分类 / 排序 / 分页、自选、净值走势和来源费率展示。5 只已验证基金支持历史与增量同步，异常数据保留原值并记录日志。持仓和模拟交易仍待实现。详见 [用户账户流程](docs/用户账户流程.md) 和 [基金目录与数据展示](docs/基金目录与数据展示.md)。
