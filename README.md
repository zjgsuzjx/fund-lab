...
> **当前进度：账户、基金浏览与模拟买卖闭环已接通。** 注册后获得一次 100,000 元虚拟本金；000147 支持买入、FIFO 分批卖出、份额冻结、撤单、正式净值确认和赎回到账。持仓区分现金、买入在途、赎回在途及剩余份额，并计算含交易费的净值收益。分红权益处理尚未实现，遇到相关事件暂停赎回。详见 [模拟买入与确认结算](docs/模拟买入与确认结算.md)及[模拟卖出与资金到账](docs/模拟卖出与资金到账.md)。
