...
> **当前进度：账户、基金浏览与模拟买卖闭环已接通。** 注册后获得一次 100,000 元虚拟本金；000147 支持买入、FIFO 分批卖出、份额冻结、撤单、正式净值确认和赎回到账。持仓区分现金、买入在途、赎回在途及剩余份额，并计算含交易费的净值收益。已增加经公告核验的现金分红登记、待到账与到账账本，以及 000147 后台定期更新；未知权益继续等待核验。详见 [现金分红与数据维护](docs/现金分红与数据维护.md)。详见 [模拟买入与确认结算](docs/模拟买入与确认结算.md)及[模拟卖出与资金到账](docs/模拟卖出与资金到账.md)。
...
阶段 1 的可启动工程和基础表已完成本机验收；原规划中的交易规则、订单、批次及任务表尚未建立；本次已新增本金资金流水表，其余交易账本将随相应业务规则一起迁移。阶段 0 的完整交易规则仍待核验，阶段 2 用户账户流程及阶段 3 基金目录、详情、自选与同步已实现；阶段 4～7 尚未完成。
